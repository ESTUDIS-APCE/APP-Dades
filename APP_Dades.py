...
if selected == "Espanya":
    ...
    if selected_type=="Indicadors macroeconòmics i financers":
        ...
        if selected_index=="Índex de Preus al Consum (IPC)":
            ...
            table_espanya_m = tidy_Catalunya_mensual(DT_monthly, ["Fecha", "IPC_Nacional_x", "IPC_subyacente", "IGC_Nacional"], f"{str(min_year)}-01-01", f"{str(max_year+1)}-01-01",["Data","IPC","IPC subjacent", "IGC"])
            table_espanya_m["Inflació"] = table_espanya_m["IPC"].pct_change(12).mul(100)
            ...
            if selected_year_n!=max_year:
                left, center, right= st.columns((1,1,1))
                with left:
                    st.metric(label="**Inflació** (Variació anual IPC)", value=f"""{round(table_espanya_y[table_espanya_y.index==str(selected_year_n)]["Inflació"].values[0], 1)}%""")
                with center:
                    st.metric(label="**Inflació subjacent** (Exclou energia i aliments frecos)", value=f"""{round(table_espanya_y[table_espanya_y.index==str(selected_year_n)]["Inflació subjacent"].values[0], 1)}%""")
                with right:
                    st.metric(label="**Índex de Garantia de Competitivitat** (Variació anual)", value=f"""{round(table_espanya_y[table_espanya_y.index==str(selected_year_n)]["Índex de Garantia de Competitivitat"].values[0], 1)}%""")

            left, right = st.columns((1,1))
            with left: 
                st.markdown("**Dades mensuals**")
                st.dataframe(table_espanya_m.reset_index().drop("Fecha", axis=1).set_index("Data"))
                st.markdown(filedownload(table_espanya_m, f"{selected_index}_Espanya.xlsx"), unsafe_allow_html=True)
            ...
            with right:
                ...
                st.markdown(filedownload(table_espanya_y, f"{selected_index}_Espanya_anual.xlsx"), unsafe_allow_html=True)
        if selected_index=="Consum de ciment":
            ...
            with left:
                st.markdown("**Dades mensuals**")
                st.dataframe(table_espanya_m.set_index("Fecha"))
                st.markdown(filedownload(table_espanya_m.set_index("Fecha"), f"{selected_index}.xlsx"), unsafe_allow_html=True)
            with center:
                st.markdown("**Dades trimestrals**")
                st.dataframe(table_espanya_q)
                st.markdown(filedownload(table_espanya_q, f"{selected_index}_Espanya.xlsx"), unsafe_allow_html=True)
            with right:
                st.markdown("**Dades anuals**")
                st.dataframe(table_espanya_y)
                st.markdown(filedownload(table_espanya_y, f"{selected_index}_Espanya_anual.xlsx"), unsafe_allow_html=True)
            left, right = st.columns((1,1))
            with left:
                st.plotly_chart(line_plotly(table_espanya_m.set_index("Fecha"), ["Consum de ciment"], "Consum de ciment (Milers T.)", "Milers de T."), use_container_width=True, responsive=True)
            with right:
                st.plotly_chart(line_plotly(table_espanya_m.set_index("Fecha").pct_change(12).mul(100).dropna(axis=0), ["Consum de ciment"], "Variació anual del consum de ciment (Milers T.)", "%"), use_container_width=True, responsive=True)     
        if selected_index=="Tipus d'interès":
            ...
            table_espanya_m = table_espanya_m[["Data","Euríbor a 1 mes","Euríbor a 3 mesos","Euríbor a 6 mesos","Euríbor a 1 any", "Tipus d'interès d'hipoteques"]].rename(columns={"Data":"Fecha"})
            ...
            with right:
                ...
                st.markdown(filedownload(table_espanya_q, f"{selected_index}_Espanya_anual.xlsx"), unsafe_allow_html=True)
    ...
    if selected_type=="Sector residencial":
        ...
        if selected_index=="Producció":
            ...
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_esp.columns.tolist(), default=table_esp.columns.tolist())
            ...
            with left:
                ...
                st.markdown(filedownload(table_esp, f"{selected_index}_Espanya.xlsx"), unsafe_allow_html=True)
                ...
                st.plotly_chart(bar_plotly(table_esp_y, selected_columns_aux1, "Qualificacions provisionals de protecció oficial segons tipus de promotor", "Nombre d'habitatges", 2014), use_container_width=True, responsive=True)
            with right:
                st.markdown("**Dades anuals**")
                st.dataframe(table_esp_y[selected_columns])
                st.markdown(filedownload(table_esp_y, f"{selected_index}_Espanya_anual.xlsx"), unsafe_allow_html=True)
                ...
                st.plotly_chart(bar_plotly(table_esp_y, selected_columns_aux2, "Qualificacions definitives de protecció oficial segons tipus de promotor", "Nombre d'habitatges", 2014), use_container_width=True, responsive=True)
        if selected_index=="Compravendes":
            ...
            with left_col:
                ...
                st.dataframe(table_esp[selected_columns])
                st.markdown(filedownload(table_esp, f"{selected_index}_Espanya.xlsx"), unsafe_allow_html=True)
                st.plotly_chart(line_plotly(table_esp[table_esp.notna()], selected_columns, "Compravendes d'habitatge per tipologia d'habitatge", "Nombre de compravendes"), use_container_width=True, responsive=True)
            with right_col:
                st.markdown("**Dades anuals**")
                st.dataframe(table_esp_y[selected_columns])
                st.markdown(filedownload(table_esp_y, f"{selected_index}_Espanya_anual.xlsx"), unsafe_allow_html=True)
                st.plotly_chart(bar_plotly(table_esp_y[table_esp_y.notna()], selected_columns, "Compravendes d'habitatge per tipologia d'habitatge", "Nombre de compravendes", 2008), use_container_width=True, responsive=True)
        if selected_index=="Preus":
                min_year=2008
                max_year=datetime.now().year  
                st.subheader("PREUS PER M\u00b2 (MITMA)")
                ...
                left, right = st.columns((1,1))
                with left:
                    st.metric(label=f"""**Preu de l'habitatge lliure** (€/m\u00b2)""", value=f"""{indicator_year(table_esp_y, table_esp, str(selected_year_n), "Preu de l'habitatge lliure", "level")} €/m\u00b2""")
                with right:
                    st.metric(label=f"""**Preu de l'habitatge lliure nou** (€/m\u00b2)""", value=f"""{round(indicator_year(table_esp_y, table_esp, str(selected_year_n), "Preu de l'habitatge lliure nou", "level"),1)} €/m\u00b2""")
                selected_columns = st.multiselect("**Selecció d'indicadors:**", table_esp.columns.tolist(), default=table_esp.columns.tolist())
                left_col, right_col = st.columns((1,1))
                with left_col:
                    st.markdown("**Dades trimestrals**")
                    st.dataframe(table_esp[selected_columns])
                    st.markdown(filedownload(table_esp, f"{selected_index}_Espanya.xlsx"), unsafe_allow_html=True)
                    st.plotly_chart(line_plotly(table_esp, selected_columns, "Preus per m\u00b2 per tipologia d'habitatge", "€/m\u00b2"), use_container_width=True, responsive=True)
                with right_col:
                    st.markdown("**Dades anuals**")
                    st.dataframe(table_esp_y[selected_columns])
                    st.markdown(filedownload(table_esp_y, f"{selected_index}_Espanya_anual.xlsx"), unsafe_allow_html=True)
                    st.plotly_chart(bar_plotly(table_esp_y, selected_columns, "Preus per m\u00b2 per tipologia d'habitatge", "€/m\u00b2", 2010), use_container_width=True, responsive=True)
                ...
                with left_col:
                    ...
                    st.dataframe(table_esp[selected_columns])
                    st.markdown(filedownload(table_esp, f"{selected_index}_Espanya.xlsx"), unsafe_allow_html=True)
                    st.plotly_chart(line_plotly(table_esp, selected_columns, "Preus per m\u00b2 per tipologia d'habitatge (variació anual %)", "%"), use_container_width=True, responsive=True)
                with right_col:
                    st.markdown("**Dades anuals**")
                    st.dataframe(table_esp_y[selected_columns])
                    st.markdown(filedownload(table_esp_y, f"{selected_index}_Espanya_anual.xlsx"), unsafe_allow_html=True)
                    st.plotly_chart(bar_plotly(table_esp_y, selected_columns, "Preus per m\u00b2 per tipologia d'habitatge (variació anual %)", "%", 2007), use_container_width=True, responsive=True)
...
if selected == "Catalunya":
    ...
    if selected_indicator=="Indicadors macroeconòmics i financers":
        ...
        if selected_index=="Mercat laboral":
            ...
            with left:
                ...
                st.markdown(filedownload(table_catalunya_q, f"{selected_index}_Catalunya.xlsx"), unsafe_allow_html=True)
            ...
            with right:
                ...
                st.markdown(filedownload(table_catalunya_y, f"{selected_index}_Catalunya_anual.xlsx"), unsafe_allow_html=True)
        ...
        if selected_index=="Costos de construcció":
            ...
            left_1, left_2, right_1, right_2 = st.columns((1,1,1,1))
            with left_1:
                st.metric(label="**Edifici renda normal entre mitjaneres** (€/m\u00b2)", value=f"""{indicator_year(table_catalunya_y, table_catalunya_q, str(selected_year_n), "Edifici renda normal entre mitjaneres", "level"):,.0f} €/m\u00b2""", delta=f"""{indicator_year(table_catalunya_y, table_catalunya_q, str(selected_year_n), "Edifici renda normal entre mitjaneres", "var")}%""")
            with left_2:
                st.metric(label="**Unifamiliar de dos plantes entre mitjaneres** (€/m\u00b2)", value=f"""{indicator_year(table_catalunya_y, table_catalunya_q, str(selected_year_n), "Unifamiliar de dos plantes entre mitjaneres", "level"):,.0f} €/m\u00b2""", delta=f"""{indicator_year(table_catalunya_y, table_catalunya_q, str(selected_year_n), "Unifamiliar de dos plantes entre mitjaneres", "var")}%""")
            with right_1:
                st.metric(label="**Nau industrial** (€/m\u00b2)", value=f"""{indicator_year(table_catalunya_y, table_catalunya_q, str(selected_year_n), "Nau industrial", "level"):,.0f} €/m\u00b2""", delta=f"""{indicator_year(table_catalunya_y, table_catalunya_q, str(selected_year_n), "Nau industrial", "var")}%""")
            with right_2:
                st.metric(label="**Edifici d’oficines entre mitjaneres** (€/m\u00b2)", value=f"""{indicator_year(table_catalunya_y, table_catalunya_q, str(selected_year_n), "Edifici d’oficines entre mitjaneres", "level"):,.0f} €/m\u00b2""", delta=f"""{indicator_year(table_catalunya_y, table_catalunya_q, str(selected_year_n), "Edifici d’oficines entre mitjaneres", "var")}%""")
            st.markdown("""Els preus per m\u00b2 construït inclouen l’estudi de seguretat i salut, els honoraris tècnics i permisos d’obra amb un benefici industrial del 20% i despeses generals. Addicionalment, 
                        cal comentar que aquests preus fan referència a la província de Barcelona. Si la ubicació de l'obra es troba en una província diferent, la disminució dels preus serà d'un 6% a 8% a Girona, 8% a 10% a Tarragona i del 12% a 15% a Lleida. Pot consultar l'última edició del Butlletí Econòmic de la Costrucció (BEC) fent click sobre el link a continuació:""")
            st.markdown(f"""<a href="https://drive.google.com/file/d/1AiS_Jb8EN9acl6vvfqPdxk0Gkg9cbUyw/view?usp=drive_link" target="_blank"><button class="download-button">Descarregar BEC</button></a>""", unsafe_allow_html=True)
            left, right = st.columns((1,1))
            with left:
                st.markdown("**Dades trimestrals**")
                st.dataframe(table_catalunya_q)
    ...
    if selected_indicator=="Sector residencial":
        ...
        if selected_type=="Venda":
            ...
            if selected_index=="Producció":
                min_year=2008
                st.subheader("PRODUCCIÓ D'HABITATGES")
                ...
                with left_col:
                    ...
                    st.markdown(filedownload(table_Catalunya, f"{selected_index}_{selected_geo}.xlsx"), unsafe_allow_html=True)
                ...
                with right_col:
                    ...
                    st.markdown(filedownload(table_Catalunya_y, f"{selected_index}_{selected_geo}_anual.xlsx"), unsafe_allow_html=True)
            ...
            if selected_index=="Compravendes":
                min_year=2014
                st.subheader("COMPRAVENDES D'HABITATGES")
            ...
            if selected_index=="Preus":
                ...
                left, center, right = st.columns((1,1,1))
                with left:
                    st.metric(label="**Preu d'habitatge total**", value=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Preu d'habitatge total", "level"):,.0f}""", delta=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Preu d'habitatge total", "var")}%""")
                with center:
                    st.metric(label="**Preu d'habitatge de segona mà**", value=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Preus d'habitatge de segona mà", "level"):,.0f}""", delta=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Preus d'habitatge de segona mà", "var")}%""")
                with right:
                    st.metric(label="**Preu d'habitatge nou**", value=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Preus d'habitatge nou", "level"):,.0f}""", delta=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Preus d'habitatge nou", "var")}%""")
                selected_columns = st.multiselect("**Selecció d'indicadors:**", table_Catalunya.columns.tolist(), default=table_Catalunya.columns.tolist())
                left_col, right_col = st.columns((1,1))
                with left_col:
                    st.markdown("**Dades trimestrals**")
                    st.dataframe(table_Catalunya[selected_columns])
                    st.markdown(filedownload(table_Catalunya, f"{selected_index}_{selected_geo}.xlsx"), unsafe_allow_html=True)
                    st.plotly_chart(line_plotly(table_Catalunya, selected_columns, "Preus per m\u00b2 per tipologia d'habitatge", "€/m\u00b2"), use_container_width=True, responsive=True)
                with right_col:
                    st.markdown("**Dades anuals**")
                    st.dataframe(table_Catalunya_y[selected_columns])
                    st.markdown(filedownload(table_Catalunya_y, f"{selected_index}_{selected_geo}_anual.xlsx"), unsafe_allow_html=True)
            ...
            if selected_index=="Superfície":
                ...
                left, center, right = st.columns((1,1,1))
                with left:
                    st.metric(label="**Superfície mitjana total** (m\u00b2)", value=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Superfície mitjana total", "level"):,.0f} m\u00b2""", delta=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Superfície mitjana total", "var")}%""")
                with center:
                    st.metric(label="**Superfície mitjana d'habitatge de segona mà** (m\u00b2)", value=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Superfície mitjana d'habitatge de segona mà", "level"):,.0f} m\u00b2""", delta=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Superfície mitjana d'habitatge de segona mà", "var")}%""")
                with right:
                    st.metric(label="**Superfície mitjana d'habitatge nou** (m\u00b2)", value=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Superfície mitjana d'habitatge nou", "level"):,.0f} m\u00b2""", delta=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Superfície mitjana d'habitatge nou", "var")}%""")
                selected_columns = st.multiselect("**Selecció d'indicadors:**", table_Catalunya.columns.tolist(), default=table_Catalunya.columns.tolist())
                left_col, right_col = st.columns((1,1))
                with left_col:
                    st.markdown("**Dades trimestrals**")
        ...
        if selected_type=="Lloguer":
            ...
            with right_col:
                st.metric(label="**Rendes mitjanes de lloguer** (€/mes)", value=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Rendes mitjanes de lloguer", "level"):,.0f} €/mes""", delta=f"""{indicator_year(table_Catalunya_y, table_Catalunya, str(selected_year_n), "Rendes mitjanes de lloguer", "var")}%""")
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_Catalunya.columns.tolist(), default=table_Catalunya.columns.tolist())
            left_col, right_col = st.columns((1,1))
            with left_col:
                st.markdown("**Dades trimestrals**")
                st.dataframe(table_Catalunya[selected_columns])
                st.markdown(filedownload(table_Catalunya, "Mercat de lloguer a Catalunya.xlsx"), unsafe_allow_html=True)
            ...
            with right_col:
                st.markdown("**Dades anuals**")
                st.dataframe(table_Catalunya_y[selected_columns])
                st.markdown(filedownload(table_Catalunya_y, "Mercat de lloguer a Catalunya.xlsx"), unsafe_allow_html=True)
...
if selected == "Províncies i àmbits":
    ...
    if selected_type=="Venda":
        ...
        if selected_option=="Àmbits territorials":
            ...
            if selected_index=="Producció":
                ...
                selected_columns = st.multiselect("**Selecció d'indicadors:**", table_province.columns.tolist(), default=table_province.columns.tolist())
                ...
                with left_col:
                    ...
                    st.markdown(filedownload(table_province, f"{selected_index}_{selected_geo}.xlsx"), unsafe_allow_html=True)
                ...
                with right_col:
                    ...
                    st.markdown(filedownload(table_province_y, f"{selected_index}_{selected_geo}_anual.xlsx"), unsafe_allow_html=True)
            ...
            if selected_index=="Preus":
                min_year=2014
                st.subheader(f"PREUS PER M\u00b2 ÚTIL D'HABITATGE A L'ÀMBIT: {selected_geo.upper()}")
                ...
                left, center, right = st.columns((1,1,1))
                with left:
                    st.metric(label="**Preu d'habitatge total**", value=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Preu d'habitatge total", "level"):,.0f}""", delta=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Preu d'habitatge total", "var")}%""")
                with center:
                    st.metric(label="**Preus d'habitatge de segona mà**", value=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Preus d'habitatge de segona mà", "level"):,.0f}""", delta=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Preus d'habitatge de segona mà", "var")}%""")
                with right:
                    st.metric(label="**Preus d'habitatge nou**", value=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Preus d'habitatge nou", "level"):,.0f}""", delta=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Preus d'habitatge nou", "var")}%""") 
                selected_columns = st.multiselect("**Selecció d'indicadors:**", table_province.columns.tolist(), default=table_province.columns.tolist())
                left_col, right_col = st.columns((1,1))
                with left_col:
                    st.markdown("**Dades trimestrals**")
                    st.dataframe(table_province[selected_columns])
                    st.markdown(filedownload(table_province, f"{selected_index}_{selected_geo}.xlsx"), unsafe_allow_html=True)
                    st.plotly_chart(line_plotly(table_province, selected_columns, "Preus per m\u00b2 per tipologia d'habitatge", "€/m\u00b2 útil"), use_container_width=True, responsive=True)
                with right_col:
                    st.markdown("**Dades anuals**")
                    st.dataframe(table_province_y[selected_columns])
                    st.markdown(filedownload(table_province_y, f"{selected_index}_{selected_geo}_anual.xlsx"), unsafe_allow_html=True)
            ...
            if selected_index=="Superfície":
                ...
                left, center, right = st.columns((1,1,1))
                with left:
                    st.metric(label="**Superfície mitjana total**", value=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Superfície mitjana total", "level"):,.0f}""", delta=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Superfície mitjana total", "var")}%""")
                with center:
                    st.metric(label="**Superfície mitjana d'habitatge de segona mà**", value=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Superfície mitjana d'habitatge de segona mà", "level"):,.0f}""", delta=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Superfície mitjana d'habitatge de segona mà", "var")}%""")
                with right:
                    st.metric(label="**Superfície mitjana d'habitatge nou**", value=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Superfície mitjana d'habitatge nou", "level"):,.0f}""", delta=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Superfície mitjana d'habitatge nou", "var")}%""") 
                selected_columns = st.multiselect("**Selecció d'indicadors:**", table_province.columns.tolist(), default=table_province.columns.tolist())
                left_col, right_col = st.columns((1,1))
                with left_col:
                    st.markdown("**Dades trimestrals**")
        ...
        if selected_option=="Províncies":
            ...
            if selected_index=="Preus":
                min_year=2014
                st.subheader(f"PREUS PER M\u00b2 ÚTIL D'HABITATGE A {selected_geo.upper()}")
            ...
            if selected_index=="Superfície":
                ...
                with left_col:
                    ...
                    st.dataframe(table_province[selected_columns])
                    st.markdown(filedownload(table_province, f"{selected_index}_{selected_geo}.xlsx"), unsafe_allow_html=True)
                    st.plotly_chart(line_plotly(table_province, selected_columns, "Superfície mitjana per tipologia d'habitatge", "m\u00b2 útil"), use_container_width=True, responsive=True)
                with right_col:
                    st.markdown("**Dades anuals**")
                    st.dataframe(table_province_y[selected_columns])
                    st.markdown("")
                    ...
                    st.markdown("")
                    st.markdown(filedownload(table_province_y, f"{selected_index}_{selected_geo}_anual.xlsx"), unsafe_allow_html=True)
                    st.plotly_chart(bar_plotly(table_province_y, selected_columns, "Superfície mitjana per tipologia d'habitatge", "m\u00b2 útil", 2005), use_container_width=True, responsive=True)
    ...
    if selected_type=="Lloguer":
        ...
        if selected_option=="Àmbits territorials":
            ...
            with right_col:
                st.metric(label="**Rendes mitjanes de lloguer** (€/mes)", value=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Rendes mitjanes de lloguer", "level"):,.0f} €/mes""", delta=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Rendes mitjanes de lloguer", "var")}%""")
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_province.columns.tolist(), default=table_province.columns.tolist())
            left_col, right_col = st.columns((1,1))
            with left_col:
                st.markdown("**Dades trimestrals**")
                st.dataframe(table_province[selected_columns])
                st.markdown(filedownload(table_province, f"Lloguer_{selected_geo}.xlsx"), unsafe_allow_html=True)
            ...
            with right_col:
                st.markdown("**Dades anuals**")
                st.dataframe(table_province_y[selected_columns])
                st.markdown(filedownload(table_province_y, f"Lloguer_{selected_geo}_anual.xlsx"), unsafe_allow_html=True)
    ...
    if selected_type=="Venda":
        ...
        if selected_index=="Producció":
            ...
            with right:
                try:
                    ...
                    st.metric(label="**Habitatges acabats unifamiliars**", value="Pendent")
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_com.columns.tolist(), default=table_com.columns.tolist())
            ...
            with left_col:
                ...
                st.markdown(filedownload(table_com[selected_columns_aux], f"{selected_index}_{selected_com}.xlsx"), unsafe_allow_html=True)
            ...
            with right_col:
                ...
                st.markdown(filedownload(table_com_y[selected_columns_aux], f"{selected_index}_{selected_com}_anual.xlsx"), unsafe_allow_html=True)
        ...
        if selected_index=="Compravendes":
            ...
            with left_col:
                st.markdown("**Dades trimestrals**")
                st.dataframe(table_com[selected_columns])
                st.markdown(filedownload(table_com, f"{selected_index}_{selected_com}.xlsx"), unsafe_allow_html=True)
            ...
            with right_col:
                ...
                st.markdown("")
                st.markdown("")
                st.markdown(filedownload(table_com_y, f"{selected_index}_{selected_com}_anual.xlsx"), unsafe_allow_html=True)
        ...
        if selected_index=="Preus":
            min_year=2014
            st.subheader(f"PREUS PER M\u00b2 ÚTIL D'HABITATGE A LA COMARCA: {selected_com.upper()}")
            ...
            left, center, right = st.columns((1,1,1))
            with left:
                st.metric(label="**Preu d'habitatge total**", value=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Preu d'habitatge total", "level"):,.0f}""", delta=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Preu d'habitatge total", "var")}%""")
            with center:
                st.metric(label="**Preu d'habitatge de segona mà**", value=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Preu d'habitatge de segona mà", "level"):,.0f}""", delta=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Preu d'habitatge de segona mà", "var")}%""")
            with right:
                try:
                    st.metric(label="**Preu d'habitatge nou**", value=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Preu d'habitatge nou", "level"):,.0f}""", delta=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Preu d'habitatge nou", "var")}%""") 
                except IndexError:
                    st.metric(label="Preu d'habitatge nou", value=0)
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_com.columns.tolist(), default=table_com.columns.tolist())
            left_col, right_col = st.columns((1,1))
            with left_col:
                st.markdown("**Dades trimestrals**")
                st.dataframe(table_com[selected_columns])
                st.markdown(filedownload(table_com, f"{selected_index}_{selected_com}.xlsx"), unsafe_allow_html=True)
                st.plotly_chart(line_plotly(table_com, selected_columns, "Preus per m\u00b2 per tipologia d'habitatge", "€/m\u00b2 útil"), use_container_width=True, responsive=True)
            with right_col:
                st.markdown("**Dades anuals**")
                st.dataframe(table_com_y[selected_columns])
                st.markdown("")
        ...
        if selected_index=="Superfície":
            ...
            left, center, right = st.columns((1,1,1))
            with left:
                st.metric(label="**Superfície mitjana total**", value=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Superfície mitjana total", "level"):,.0f}""", delta=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Superfície mitjana total", "var")}%""")
            with center:
                st.metric(label="**Superfície mitjana d'habitatge de segona mà**", value=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Superfície mitjana d'habitatge de segona mà", "level"):,.0f}""", delta=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Superfície mitjana d'habitatge de segona mà", "var")}%""")
            with right:
                try:
                    st.metric(label="**Superfície mitjana d'habitatge nou**", value=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Superfície mitjana d'habitatge nou", "level"):,.0f}""", delta=f"""{indicator_year(table_com_y, table_com, str(selected_year_n), "Superfície mitjana d'habitatge nou", "var")}%""") 
                except IndexError:
                    st.metric(label="**Superfície mitjana d'habitatge nou**", value=0)
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_com.columns.tolist(), default=table_com.columns.tolist())
            left_col, right_col = st.columns((1,1))
            with left_col:
                st.markdown("**Dades trimestrals**")
                st.dataframe(table_com[selected_columns])
                st.markdown(filedownload(table_com, f"{selected_index}_{selected_com}.xlsx"), unsafe_allow_html=True)
                st.plotly_chart(line_plotly(table_com, selected_columns, "Superfície mitjana per tipologia d'habitatge", "m\u00b2 útil"), use_container_width=True, responsive=True)
            with right_col:
                st.markdown("**Dades anuals**")
                st.dataframe(table_com_y[selected_columns])
                st.markdown(filedownload(table_com_y, f"{selected_index}_{selected_com}.xlsx"), unsafe_allow_html=True)
                st.plotly_chart(bar_plotly(table_com_y, selected_columns, "Superfície mitjana per tipologia d'habitatge", "m\u00b2 útil", 2005), use_container_width=True, responsive=True)
    if selected_type=="Lloguer":
        ambit_names = ["Alt Pirineu i Aran","Camp de Tarragona","Comarques centrals","Comarques gironines","Metropolità","Penedès","Ponent","Terres de l'Ebre"]
        selected_com = st.sidebar.selectbox("**Selecciona una comarca:**", maestro_mun["Comarca"].unique(), index= maestro_mun["Comarca"].unique().tolist().index("Barcelonès"))
        available_years = list(range(2015, datetime.now().year + 1))
        ...
        with left_col:
            st.markdown("**Dades trimestrals**")
            st.dataframe(table_province[selected_columns])
            st.markdown(filedownload(table_province, f"Lloguer_{selected_com}.xlsx"), unsafe_allow_html=True)
            st.plotly_chart(line_plotly(table_province, ["Rendes mitjanes de lloguer"], "Evolució trimestral de les rendes mitjanes de lloguer", "€/mes"), use_container_width=True, responsive=True)
        ...
        with right_col:
            st.markdown("**Dades anuals**")
            st.dataframe(table_province_y[selected_columns])
            st.markdown(filedownload(table_province_y, f"Lloguer_{selected_com}.xlsx"), unsafe_allow_html=True)
            st.plotly_chart(bar_plotly(table_province_y, ["Rendes mitjanes de lloguer"], "Evolució anual de les rendes mitjanes de lloguer", "€/mes", 2005), use_container_width=True, responsive=True)
...
if selected=="Municipis":
    ...
    if selected_type=="Venda":
        ...
        if selected_index=="Producció":
            ...
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_mun.columns.tolist(), default=table_mun.columns.tolist())
            ...
            with left_col:
                ...
                st.markdown(filedownload(table_mun[selected_columns_aux], f"{selected_index}_{selected_mun}.xlsx"), unsafe_allow_html=True)
            ...
            with right_col:
                ...
                st.markdown(filedownload(table_mun_y[selected_columns_aux], f"{selected_index}_{selected_mun}_anual.xlsx"), unsafe_allow_html=True)
        ...
        if selected_index=="Compravendes":
            ...
            with left_col:
                st.markdown("**Dades trimestrals**")
                st.dataframe(table_mun[selected_columns])
                st.markdown(filedownload(table_mun, f"{selected_index}_{selected_mun}.xlsx"), unsafe_allow_html=True)
        ...
        if selected_index=="Preus":
            ...
            with left:
                try:
                    st.metric(label="**Preu d'habitatge total** (€/m\u00b2 construït)", value=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Preu d'habitatge total", "level"):,.0f} €/m\u00b2""", delta=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Preu d'habitatge total", "var")}%""")
                except IndexError:
                    st.metric(label="**Preu d'habitatge total** (€/m\u00b2 construït)", value="n/a") 
            with center:
                try:
                    st.metric(label="**Preu d'habitatge de segona mà** (€/m\u00b2 construït)", value=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Preu d'habitatge de segona mà", "level"):,.0f} €/m\u00b2""", delta=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Preu d'habitatge de segona mà", "var")}%""")
                except IndexError:
                    st.metric(label="**Preu d'habitatge de segona mà** (€/m\u00b2 construït)", value="n/a") 
            with right:
                try:
                    st.metric(label="**Preu d'habitatge nou** (€/m\u00b2 construït)", value=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Preu d'habitatge nou", "level"):,.0f} €/m\u00b2""", delta=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Preu d'habitatge nou", "var")}%""") 
                except IndexError:
            ...
            with left_col:
                ...
                st.dataframe(table_mun[selected_columns])
                st.markdown(filedownload(table_mun, f"{selected_index}_{selected_mun}.xlsx"), unsafe_allow_html=True)
                st.plotly_chart(line_plotly(table_mun, selected_columns, "Preus per m\u00b2 per tipologia d'habitatge", "€/m\u00b2 útil"), use_container_width=True, responsive=True)
            with right_col:
                st.markdown("**Dades anuals**")
                st.dataframe(table_mun_y[selected_columns])
                st.markdown("")
        ...
        if selected_index=="Superfície":
            ...
            with left:
                try:
                    st.metric(label="**Superfície mitjana total** (m\u00b2 construït)", value=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Superfície mitjana total", "level"):,.0f} m\u00b2""", delta=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Superfície mitjana total", "var")}%""")
                except IndexError:
                    st.metric(label="**Superfície mitjana total** (m\u00b2 construït)", value="n/a")
            with center:
                try:
                    st.metric(label="**Superfície mitjana d'habitatge de segona mà** (m\u00b2 construït)", value=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Superfície mitjana d'habitatge de segona mà", "level"):,.0f} m\u00b2""", delta=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Superfície mitjana d'habitatge de segona mà", "var")}%""")
                except IndexError:
                    st.metric(label="**Superfície mitjana d'habitatge de segona mà** (m\u00b2 construït)", value="n/a")
            with right:
                try:
                    st.metric(label="**Superfície mitjana d'habitatge nou** (m\u00b2 construït)", value=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Superfície mitjana d'habitatge nou", "level"):,.0f} m\u00b2""", delta=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Superfície mitjana d'habitatge nou", "var")}%""")
                except IndexError:
                    st.metric(label="**Superfície mitjana d'habitatge nou** (m\u00b2 construït)", value="n/a")
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_mun.columns.tolist(), default=table_mun.columns.tolist())
            left_col, right_col = st.columns((1,1))
            with left_col:
                st.markdown("**Dades trimestrals**")
                st.dataframe(table_mun[selected_columns])
                st.markdown(filedownload(table_mun, f"{selected_index}_{selected_mun}.xlsx"), unsafe_allow_html=True)
                st.plotly_chart(line_plotly(table_mun, selected_columns, "Superfície mitjana per tipologia d'habitatge", "m\u00b2 útil"), use_container_width=True, responsive=True)
            with right_col:
                st.markdown("**Dades anuals**")
                st.dataframe(table_mun_y[selected_columns])
                st.markdown("")
                ...
                st.markdown("")
                st.markdown(filedownload(table_mun_y, f"{selected_index}_{selected_mun}_anual.xlsx"), unsafe_allow_html=True)
                st.plotly_chart(bar_plotly(table_mun_y, selected_columns, "Superfície mitjana per tipologia d'habitatge", "m\u00b2 útil", 2005), use_container_width=True, responsive=True)
    if selected_type=="Lloguer":
        selected_mun = st.sidebar.selectbox("**Selecciona un municipi:**", maestro_mun["Municipi"].unique(), index= maestro_mun["Municipi"].tolist().index("Barcelona"))
        max_year=datetime.now().year
        available_years = list(range(2015, datetime.now().year + 1))
        ...
        with right_col:
            try:
                st.metric(label="**Rendes mitjanes de lloguer** (€/mes)", value=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Rendes mitjanes de lloguer", "level"):,.0f} €/mes""", delta=f"""{indicator_year(table_mun_y, table_mun, str(selected_year_n), "Rendes mitjanes de lloguer", "var")}%""")
            except IndexError:
        ...
        with left_col:
            st.markdown("**Dades trimestrals**")
            st.dataframe(table_mun[selected_columns])
            st.markdown(filedownload(table_mun, f"Lloguer_{selected_mun}.xlsx"), unsafe_allow_html=True)
        ...
        with right_col:
            st.markdown("**Dades anuals**")
            st.dataframe(table_mun_y[selected_columns])
            st.markdown(filedownload(table_mun_y, f"Lloguer_{selected_mun}_anual.xlsx"), unsafe_allow_html=True)
...
if selected=="Districtes de Barcelona":
    ...
    if selected_type=="Venda":
        ...
        if selected_index=="Producció":
            ...
            with left_col:
                ...
                st.markdown(filedownload(table_dis[selected_columns_aux], f"{selected_index}_{selected_dis}.xlsx"), unsafe_allow_html=True)
            ...
            with right_col:
                ...
                st.markdown(filedownload(table_dis_y[selected_columns_aux], f"{selected_index}_{selected_dis}_anual.xlsx"), unsafe_allow_html=True)
        ...
        if selected_index=="Compravendes":
            ...
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_dis_y.columns.tolist(), default=table_dis_y.columns.tolist())
            left_col, right_col = st.columns((1,1))
            with left_col:
                st.markdown("**Dades trimestrals**")
                st.dataframe(table_dis[selected_columns])
                st.markdown(filedownload(table_dis, f"{selected_index}_{selected_dis}.xlsx"), unsafe_allow_html=True)
            ...
            with right_col:
                ...
                st.markdown("")
                st.markdown("")
                st.markdown(filedownload(table_dis_y, f"{selected_index}_{selected_dis}_anual.xlsx"), unsafe_allow_html=True)
        ...
        if selected_index=="Preus":
            ...
            left, center, right = st.columns((1,1,1))
            with left:
                st.metric(label="**Preu d'habitatge total** (€/m\u00b2)", value=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Preu d'habitatge total", "level"):,.0f} €/m\u00b2""", delta=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Preu d'habitatge total", "var")}%""")
            with center:
                st.metric(label="**Preu d'habitatge de segona mà** (€/m\u00b2)", value=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Preu d'habitatge de segona mà", "level"):,.0f} €/m\u00b2""", delta=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Preu d'habitatge de segona mà", "var")}%""")
            with right:
                st.metric(label="**Preu d'habitatge nou** (€/m\u00b2)", value=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Preu d'habitatge nou", "level"):,.0f} €/m\u00b2""", delta=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Preu d'habitatge nou", "var")}%""") 
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_dis.columns.tolist(), default=table_dis.columns.tolist())
            left_col, right_col = st.columns((1,1))
            with left_col:
                st.markdown("**Dades trimestrals**")
        ...
        if selected_index=="Superfície":
            ...
            left, center, right = st.columns((1,1,1))
            with left:
                st.metric(label="**Superfície mitjana total** (m\u00b2 construït)", value=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Superfície mitjana total", "level"):,.0f} m\u00b2""", delta=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Superfície mitjana total", "var")}%""")
            with center:
                st.metric(label="**Superfície mitjana d'habitatge de segona mà** (m\u00b2 construït)", value=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Superfície mitjana d'habitatge de segona mà", "level"):,.0f} m\u00b2""", delta=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Superfície mitjana d'habitatge de segona mà", "var")}%""")
            with right:
                st.metric(label="**Superfície mitjana d'habitatge nou** (m\u00b2 construït)", value=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Superfície mitjana d'habitatge nou", "level"):,.0f} m\u00b2""", delta=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Superfície mitjana d'habitatge nou", "var")}%""")
            selected_columns = st.multiselect("**Selecció d'indicadors:**", table_dis.columns.tolist(), default=table_dis.columns.tolist())
            left_col, right_col = st.columns((1,1))
            with left_col:
                st.markdown("**Dades trimestrals**")
    ...
    if selected_type=="Lloguer":
        ...
        with right_col:
            st.metric(label="**Rendes mitjanes de lloguer** (€/mes)", value=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Rendes mitjanes de lloguer", "level"):,.0f} €/mes""", delta=f"""{indicator_year(table_dis_y, table_dis, str(selected_year_n), "Rendes mitjanes de lloguer", "var")}%""")
        selected_columns = st.multiselect("**Selecció d'indicadors:**", table_dis.columns.tolist(), default=table_dis.columns.tolist())
        left, right = st.columns((1,1))
        with left:
            st.dataframe(table_dis[selected_columns])
            st.markdown(filedownload(table_dis, f"Lloguer_{selected_dis}.xlsx"), unsafe_allow_html=True)
        ...
        with right:
            st.dataframe(table_dis_y[selected_columns])
            st.markdown(filedownload(table_dis_y, f"Lloguer_{selected_dis}.xlsx"), unsafe_allow_html=True)
